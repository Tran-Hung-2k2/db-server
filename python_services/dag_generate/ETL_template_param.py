...
def taskflow_dag(**context):
    ...
    def extract(**context) -> pd.DataFrame:
        ...
        delta_table_path = f"s3://{extract_conf['username']}/{extract_conf['dataset']}"
        df = DeltaTable(delta_table_path, version=extract_conf['version'], storage_options=storage_options).to_pandas()
        return df
    ...
    def load(df: pd.DataFrame,**context) -> bool:
        ...
        delta_table_path = f"s3://duonghdt/dataset"
        delta_table_path = f"s3://{load_conf['username']}/{load_conf['dataset']}"
