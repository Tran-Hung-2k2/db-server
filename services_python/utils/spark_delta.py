...
LIVY_HOST = os.getenv("LIVY_HOST", "localhost")
LIVY_PORT = os.getenv("LIVY_PORT", "8089")

import requests

def get_all_active_sessions():
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions"
...
def get_all_idle_session_():
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions"
    response = requests.get(endpoint)
    
    if response.status_code == 200:
        sessions = response.json()
        idle_sessions = [session['id'] for session in sessions['sessions'] if session['state'] == "idle"] # all state is: ['not_started', 'starting', 'idle', 'busy', 'shutting_down', 'error', 'dead', 'killed', 'success']
...
def create_new_session(session_name, driverMemory=1, driverCores=1, executorMemory=4, executorCores=2, numExecutors=2):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions"
...
def get_one_active_session(session_id):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions{session_id}"
    response = requests.get(endpoint)
    if response.status_code == 200:
        return response.json()
    else:
...
def get_one_active_session_state(session_id):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions{session_id}/state"
    response = requests.get(endpoint)
    if response.status_code == 200:
        return response.json()
    else:
...
def get_one_active_session_log(session_id,offset=0,max=20):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions{session_id}/log"
    params = {
        'from': offset,
        'size': max
    }
...
def execute_statement(session_id, statement):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions{session_id}/statements"
    headers = {'Content-Type': 'application/json'}
    statement_body = {
        "code": statement
    }
...
def get_statement_output(session_id, statement_id):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions{session_id}/statements{statement_id}"
    response = requests.get(endpoint)
    if response.status_code == 200:
        return response.json()
    else:
...
def delete_one_statement(session_id, statement_id):
    endpoint = f"{LIVY_HOST}:{LIVY_PORT}/sessions{session_id}/statements{statement_id}/cancel"
